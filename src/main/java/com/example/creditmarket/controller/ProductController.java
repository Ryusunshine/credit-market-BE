...
public class ProductController {
    ...
    @GetMapping("/item/{id}")
    public ResponseEntity<ProductDetailResponseDTO> itemDetail(@PathVariable String id, HttpServletRequest request) {
        return ResponseEntity.ok().body(productService.getProductDetail(id, request));
    }

    @ApiOperation(value = "상품 구매", notes = "상품을 구매(신청)합니다.")
    @PostMapping("/order")
    public ResponseEntity<?> buyProduct(@RequestBody OrderSaveRequestDTO requestDTO, Authentication authentication) {
        String userEmail = authentication.getName();
        return ResponseEntity.ok().body(productService.buyProduct(requestDTO, userEmail));
    }

    @ApiOperation(value = "상품 추천", notes = "회원의 선호하는 대출상품과 금리에 따라 상품을 추천해줍니다")
    @GetMapping("/recommend")
    public ResponseEntity<List<RecommendResponseDTO>> recommendList(HttpServletRequest request) {
        return ResponseEntity.ok().body(productService.recommendList(request));
    }
    ...
    public ResponseEntity<?> addLike(@PathVariable String id, Authentication authentication) {
        String userEmail = authentication.getName();
        return ResponseEntity.ok().body(productService.favoriteService(id, userEmail));
    }

    @ApiOperation(value = "찜 취소", notes = "상품의 찜을 취소합니다")
    @DeleteMapping("/favor/{id}")
    public ResponseEntity<?> cancelLike(@PathVariable String id, Authentication authentication) {
        String userEmail = authentication.getName();
        return ResponseEntity.ok().body(productService.favoriteService(id, userEmail));
    }
}
